...
export class ProfileVariantComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.profileVariantService
      ...
      .subscribe((res: HttpResponse<IProfileVariant[]>) => this.paginateProfileVariants(res.body, res.headers));
  }
  ...
  transition() {
    this.router.navigate(['/profile-variant'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IProfileVariant) {
    return item.id;
  }

  registerChangeInProfileVariants() {
    this.eventSubscriber = this.eventManager.subscribe('profileVariantListModification', () => this.loadAll());
  }

  delete(profileVariant: IProfileVariant) {
  ...
  protected paginateProfileVariants(data: IProfileVariant[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.profileVariants = data;
  }
}
